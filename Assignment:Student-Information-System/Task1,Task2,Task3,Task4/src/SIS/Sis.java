...
public class Sis {
    ...
    public void enrollStudentInCourse(Student student, Course course) throws Sisexceptions.DuplicateEnrollmentException, Sisexceptions.CourseNotFoundException, Sisexceptions.StudentNotFoundException, Sisexceptions.InsufficientFundsException, InvalidEnrollmentDataException {
        ...
        if (student.getBalance() < course.getCourseFee()) {
            throw new Sisexceptions.InsufficientFundsException("Student does not have enough funds to enroll in this course.");
        }
	        
		student.enrollincourse(course);
		Enrollment enrollment=new Enrollment(enrollments.size()+1,student,course,java.time.LocalDate.now().toString());
    ...
    public void assignTeacherToCourse(Teacher teacher, Course course) throws Sisexceptions.TeacherNotFoundException {
		...
		System.out.println(teacher.getfirstName()+" "+teacher.getlastName()+" is assigned to "+course.getcourseName());
	}
	
    public void recordPayment(Student student, double amount, String paymentDate) throws Sisexceptions.PaymentValidationException {
        if (amount <= 0) {
